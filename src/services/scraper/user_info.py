...
def get_user_infos(username):
    ...
    url = f"https://{config.RAPID_API_INSTAGRAM_HOST}/v1/info"
    querystring = {
    ...
    if data:
        ...
        months = (today.year - formatted_date_joined.year) * 12 + (  # type: ignore
            today.month - formatted_date_joined.month  # type: ignore
        )
        if months < 6:
            return None
